...
void* app_Thread_Start(void* args)
{
	...
	if(isTrialLicense())
	{
    [SOApp.nowPlayingView performSelectorOnMainThread:@selector(showTrialAlert) withObject:nil waitUntilDone:YES];
	}
#endif
...
-(void)showTrialAlert
{
	UIAlertView* trialAlertView=[[UIAlertView alloc] initWithTitle:nil
                    message:@"This trial is set to\n2 minute game sessions.\nPlease consider purchasing.\nThank you so much!\n"
										delegate:self cancelButtonTitle:nil
                    otherButtonTitles:@"OK", @"PURCHASE",nil];
	[trialAlertView show];
	[trialAlertView release];
...
- (void)startEmu:(char*)path {
  ...
  iphone_menu = IPHONE_MENU_DISABLED;
	...
	ourArgs[ourArgsCnt]=get_documents_path("n64iphone"); ourArgsCnt++;

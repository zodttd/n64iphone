...
void osd_message_set_corner(osd_message_t *msg, enum osd_corner eCorner){}
void osd_message_set_static(osd_message_t *msg){}
osd_message_t * osd_message_valid(osd_message_t *msg){ return NULL; }
#endif
...
#else
  sprintf(buffer, "%s", "/Applications/n64iphone.app/");
  return true;
#endif
}
#endif
...
static char l_FilenameBuffer[PATH_MAX] = {'\0'};
#else
...
static void setPaths(void)
{
  ...
  strncpy(l_ConfigDir, "/Applications/n64iphone.app/config/", PATH_MAX);
  strncpy(l_InstallDir, "/Applications/n64iphone.app/", PATH_MAX);
  return;
...
int SDL_main(int argc, char *argv[])
{
    ...
    if (!l_GuiEnabled)
    {
      sprintf(l_FilenameBuffer, "%s", "/var/mobile/Media/ROMs/N64/rom.rom");
      l_Filename = l_FilenameBuffer;
    }
    */
